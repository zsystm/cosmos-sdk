...
type Meter = gas.Meter

type gasMeterKey struct{}
type blockGasMeterKey struct{}
...
func GetMeter(ctx context.Context) Meter {
	...
	if mgr, ok := ctx.Value(gasMeterKey{}).(Meter); ok {
		return mgr
	}

	return NewInfinteMeter()
}

func GetBlockMeter(ctx context.Context) Meter {
	sdkCtx, ok := ctx.(internalcontext.Context)
	if ok {
		return sdkCtx.BlockGasMeter
	}

	if mgr, ok := ctx.Value(blockGasMeterKey{}).(Meter); ok {
		return mgr
	}
...
func WithMeter(ctx context.Context, meter Meter) context.Context {
	sdkCtx, ok := ctx.(internalcontext.Context)
	if ok {
		sdkCtx.GasMeter = meter
		return sdkCtx
	}

	return appcontext.WithValue(ctx, gasMeterKey{}, meter)
}

func WithBlockMeter(ctx context.Context, meter Meter) context.Context {
	sdkCtx, ok := ctx.(internalcontext.Context)
	if ok {
		sdkCtx.BlockGasMeter = meter
		return sdkCtx
	}

	return appcontext.WithValue(ctx, blockGasMeterKey{}, meter)
}
...
// NewMeter returns a reference to a new basicGasMeter.
...
// GasConsumed returns the gas consumed from the Meter.
func (g *basicGasMeter) GasConsumed() Gas {
	return g.consumed
}

// GasRemaining returns the gas left in the Meter.
...
// Limit returns the gas limit of the Meter.
...
// NewInfinteMeter returns a new gas meter without a limit.
func NewInfinteMeter() Meter {
...
// GasConsumedToLimit returns the gas consumed from the Meter since the gas is not confined to a limit.
