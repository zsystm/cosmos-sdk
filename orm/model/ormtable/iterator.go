...
func prefixIterator(iteratorStore kvstore.Reader, context ReadContext, index concreteIndex, prefix []byte, options IteratorOptions) (Iterator, error) {
	if !options.Reverse {
		var start []byte
...
func rangeIterator(iteratorStore kvstore.Reader, context ReadContext, index concreteIndex, start, end []byte, fullEndKey bool, options IteratorOptions) (Iterator, error) {
	if !options.Reverse {
		if len(options.Cursor) != 0 {
