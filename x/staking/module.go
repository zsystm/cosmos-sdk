...
// ConsensusVersion defines the current x/staking module consensus version.
const consensusVersion uint64 = 4
...
func NewAppModule(cdc codec.Codec, keeper *keeper.Keeper, ak types.AccountKeeper, bk types.BankKeeper, ss exported.Subspace) AppModule {
	return AppModule{
...
func provideModule(in stakingInputs) stakingOutputs {
	k := keeper.NewKeeper(in.Cdc, in.Key, in.AccountKeeper, in.BankKeeper, authtypes.NewModuleAddress(govtypes.ModuleName).String())
